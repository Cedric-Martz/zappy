...
def build_world_state(player, uuid: UUID):
    ...
    world_state.set("elevation_underway", False)
    ...
    world_state.set("food_production_slave_amount", 40)

    world_state.set("drop_food_on_fork", False)

    world_state.set("prepared_finish", False)
    world_state.set("has_enough_stones_to_finish", False)

    world_state.set("end_player_food_amount", 35)
    world_state.set("end_player_food_taken", False)
    world_state.set("food_taken_count", 0)
    world_state.set("food_distributed", False)

    world_state.set("should_gather", False)
    world_state.set("at_goal", False)
    world_state.set("is_leader", False)
    world_state.set("vice_leader_check", False)
    world_state.set("already_set_vice_leader", False)
    world_state.set("is_vice_leader", False)
    world_state.set("should_complete_elevation", False)
    world_state.set("gather_direction", -1)
    world_state.set("waiting_for_normal_player_info", False)
    world_state.set("waited_for_elevation", False)
